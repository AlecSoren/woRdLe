...
class Runner():
  ...
  def select_action(self, state):
    ...
    if sample > eps_thresh: 
      ...
      return torch.tensor([[random.randrange(26)]], device = device, dtype=torch.long)
  ...
  def env_step(self, action):
    state, reward, done, truncated, log = env.step(action)
    state = convert_state_to_input(state)
    return torch.FloatTensor([state]).to(device), torch.FloatTensor([reward]).to(device), done or truncated, log
  ...
  def train(self, episodes=100, smooth=10): 
    ...
    for episode in range(episodes):
      ...
      state = env.reset()[0]
      state = convert_state_to_input(state)
      state = Variable(torch.from_numpy(np.array(state)).float().unsqueeze(0)).to(device)
